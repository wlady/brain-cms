...
Ext.define('Ext.ux.form.field.BoxSelect', {
	...
	// by wlady
	// this flag should be set on Enter key
	enterDetected: false,
	enterParam: 'enterDetected',
    ...
    setValue: function(value, doSelect, skipLoad) {
        ...
        if ((skipLoad !== true) && (unknownValues.length > 0) && (me.queryMode === 'remote')) {
            ...
            params[me.enterParam] = me.enterDetected;
            me.enterDetected = false;
            me.store.load({
                params: params,
                callback: function() {
                    if (me.itemList) {
    ...
    checkChange: function() {
        if (!this.suspendCheckChange && !this.isDestroyed) {
            ...
            valueField = me.valueField;
            var newValue = Ext.Array.map(Ext.Array.from(me.value), function(val) {
                if (val.isModel) {
