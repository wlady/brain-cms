...
Ext.define('Ext.ux.form.plugin.HtmlEditor', {
	...
	*/
	...
	/**
	* @cfg {Boolean} enableImages "images" plugin which allows inserting images at the cursor.
	*/
	enableImages:           false,
	/**
	...
	*/
	...
	onInitialize: function(){
		...
		if(me.enableImages || me.enableAll){
			me.editor.getToolbar().insert(me.editor.getToolbar().items.indexOfKey('createlink')+1, btn('images', false, me.doImages));
		}
		...
		if(items.length > 0){
			...
			fn = Ext.bind(me.onEditorEvent, me);
	...
	doImages: function(){
		var me = this,
			alignOptions = [['', 'None'], ['left', 'Left'], ['middle', 'Middle'], ['right', 'Right'], ['top', 'Top'], ['baseline', 'Baseline'], ['bottom', 'Bottom']],
			showCellLocationText = false,
			origWidth, origHeight;

		if (!me.imageWindow){
			me.imageWindow = new Ext.Window({
				title: me.buttonTips['images'].title,
				modal: true,
				width: 500,
				cloaseAction: 'hide',
				resizable: false,
								...
								handler: function() {
									Ext.create('module.UserFiles.ux.Window', {
										...
										callback: function(selectedRows, path) {
											var frm = me.imageWindow.down('form').getForm();
											var data = selectedRows[0].data;
											if (frm && data.path) {
												frm.findField('src').setValue(data.path);
												frm.findField('width').setValue(data.width);
												frm.findField('height').setValue(data.height);
							...
							items: [
								{
									...
									listeners: {
										change: function(el) {
											if (el.checked) {
												var frm = me.imageWindow.down('form').getForm(),
												w = parseInt(frm.findField('width').getValue()),
												h = parseInt(frm.findField('height').getValue()),
												tmpH = h;
												if (w && h) {
													if (w != origWidth) {
														// width was changed
														var originalRatio = w * 100 / origWidth;
														origWidth = w;
														origHeight = parseInt(h * originalRatio / 100);
														frm.findField('height').setValue(origHeight);
													} else if (h != origHeight) {
														// height was changed
														var originalRatio = h * 100 / origHeight;
														origHeight = h;
														origWidth = parseInt(w * originalRatio / 100);
														frm.findField('width').setValue(origWidth);
													}
												}
											}
										}
				...
				buttons: [{
					...
					handler: function(){
						var frm = me.imageWindow.down('form').getForm();
						if (frm.isValid()) {
							var src = frm.findField('src').getValue();
							var w = frm.findField('width').getValue();
							var width = w ? 'width="'+w+'"' : '';
							var h = frm.findField('height').getValue();
							var height = h ? 'height="'+h+'"' : '';
							var b = frm.findField('border').getValue();
							var border = b ? 'border="'+b+'"' : '';
							var hs = frm.findField('hspace').getValue();
							var hspace = hs ? 'hspace="'+hs+'"' : '';
							var vs = frm.findField('vspace').getValue();
							var vspace = vs ? 'vspace="'+vs+'"' : '';
							var al = frm.findField('align').getValue();
							var align = al ? 'align="'+al+'"' : '';
							var alt = frm.findField('alt').getValue();
							var html = Ext.String.format('<img src="{0}" {1} {2} {3} {4} {5} {6} alt="{7}" />', src, width, height, border, hspace, vspace, align, alt);
			...
			var frm = this.imageWindow.down('form');
			if (frm) {
				frm.getForm().reset();
			}
