...
* @adapted wlady at gmail2001 dot com
...
Ext.define('Ext.ux.upload.Basic', {
	...
	configs: {
		uploader: {
			runtimes: Ext.isIE ? 'flash,silverlight,html5' : 'html5,flash',
			url: '',
			browse_button:  Ext.id(),
			//container: null,
			max_file_size: '128mb',
			resize: '',
			flash_swf_url: '/modules/Admin/lib/plupload/js/Moxie.swf',
			silverlight_xap_url: '/modules/Admin/lib/plupload/js/Moxie.xap',
			filters: [],
			...
			statusQueuedText: __.statusQueuedText,
			statusUploadingText: __.statusUploadingText,
			statusFailedText: __.statusFailedText,
			statusDoneText: __.statusDoneText,
			statusInvalidSizeText: __.statusInvalidSizeText,
			statusInvalidExtensionText: __.statusInvalidExtensionText,
	...
	constructor: function(owner, config)
	{
		...
		me.actions = {
			...
			add: Ext.create('Ext.Action', {
				...
				iconCls: config.addButtonCls || 'plupload_iconAdd',
				disabled: false
			}),
			start: Ext.create('Ext.Action', {
				text: config.uploadButtonText || 'Start',
				disabled: true,
				iconCls: config.uploadButtonCls || 'plupload_iconUpload',
				handler: me.start,
				scope: me
			}),
			cancel: Ext.create('Ext.Action', {
				...
				disabled: true,
				iconCls: config.cancelButtonCls || 'plupload_iconCancel',
				handler: me.cancel,
				scope: me
			}),
			removeUploaded: Ext.create('Ext.Action', {
				...
				disabled: true,
				iconCls: config.removeButtonCls || 'plupload_iconClear',
			...
			removeAll: Ext.create('Ext.Action', {
				...
				disabled: true,
				iconCls: config.removeAllButtonCls || 'plupload_iconClear',
	...
	*/
	...
	{
		if (!Ext.isEmpty(this.uploadpath)) {
			uploader.settings.url = this.url+this.uploadpath;
		}
	},
