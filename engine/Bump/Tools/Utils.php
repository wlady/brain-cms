...
class Utils
{
    ...
    {
        if (($routes = CMS::Config()->cache->get('routes')) !== false) {
            return $routes;
        }
        ...
        foreach ($routers as $router) {
            $routes .= str_replace('<?php', '', file_get_contents($router));
        }
        $stmts = $parser->parse('<?php'.$routes);
        $routes = [];
        foreach ($stmts as $stmt) {
            if ($comment = $stmt->getDocComment()) {
                if (preg_match('~@Route\((.*)\)~is', (string)$comment, $found)) {
                    $method = strtoupper($stmt->name);
                    if ($method == 'VIA') {
                        $url = $stmt->var->args[0]->value->value;
                        $methods = [];
                        foreach ($stmt->args as $arg){
                            $methods[] = strtoupper($arg->value->value);
                        }
                        $method = implode(', ', $methods);
                    } elseif (in_array($method, ['GET', 'POST'])) {
                        $url = $stmt->args[0]->value->value;
                    } else {
                        $method = false;
                    }
                    if ($method) {
        ...
        // save to DB
        $router = new \Bump\Modules\Routes\Routes();
        $router->saveRows($routes);
        $seoMetaData = new \Bump\Modules\SeoMetaDatas\SeoMetaDatas();
        $isAjax = CMS::Request()->isAjax();
        foreach ($routes as &$route) {
            if ($row = $seoMetaData->getRow([
                'url' => $route['url']
            ])
            ) {
    ...
    public static function rewriteCssUrls($css)
    {
        if (self::isNonCacheableUrl()) {
            return $css;
        }
        $cdns = CMS::Config()->cdns;
        $pattern = "/url\(\s*[\"']?([^\s\"']+?)[\"']?\s*\)/";
    ...
    public static function rewriteImgUrls($html)
    {
        if (self::isNonCacheableUrl()) {
            return $html;
        }
    ...
    public static function isNonCacheableUrl()
    {
        $res = false;
        $paths = parse_url($_SERVER['REQUEST_URI']);
        if (($noncacheable = CMS::config()->noncacheable)) {
            foreach ((array)$noncacheable as $mask) {
                if (preg_match("!{$mask}!", $paths['path'])) {
                    $res = true;
                    break;
                }
            }
    ...
    public static function sizeInBytes($val)
    {
        $val = trim($val);
    ...
    public static function getJSDataStore($module, &$dataStores)
    {
        if (class_exists($module)) {
            $reflection = new \ReflectionClass($module);
            $rcms = $reflection->getMethods(\ReflectionMethod::IS_PUBLIC);
            foreach ($rcms as $rcm) {
                $args = [];
                $ans = $rcm->getDocComment();
                $lines = explode('@JSDataStore', $ans);
                foreach ($lines as $line) {
                    if (preg_match('~^\((.*)\)~', trim($line), $found)) {
                        $args = [];
                        if (preg_match_all('~(\w+):=*[^:=]~i', $found[1], $mm, PREG_OFFSET_CAPTURE)) {
                            $nums = count($mm[1]);
                            foreach ($mm[1] as $key => $offs) {
                                $attr = $offs[0];
                                $len = strlen($attr);
                                if ($key < $nums - 1) {
                                    $val = substr($found[1], $offs[1] + $len + 2,
                                        $mm[1][$key + 1][1] - $offs[1] - $len - 2);
                                } else {
                                    $val = substr($found[1], $offs[1] + $len + 2);
                                }
                                $val = trim($val);
                        ...
                        $res = call_user_func([new $module, $rcm->getName()], $args);
                        $_REQUEST = $_TMP_REQUEST;
                        if (isset($res['fields']) && isset($res['data'])) {
                            $data = $res['data'];
                            $fields = $res['fields'];
                        } else {
                            $data = $res;
                        ...
                        $dataStores[$id] = [
                            ...
                            'module' => $reflection->getShortName(),
                            'action' => $rcm->getName()
                        ];
                    }
                }
    ...
    public static function loadResourceFile($filename, &$resources)
    {
        if (file_exists($filename)) {
            $res = file($filename);
            $data = array_filter($res, 'getNonCommentedLine');
            if (count($data)) {
    ...
    public static function saveStaticFile($propID, $url)
    {
        $picsLibrary = PROPERTIES_IMAGES_DIR . $propID;
        if (!is_dir($picsLibrary)) {
            mkdir($picsLibrary);
        }
        $parts = explode('/', $url);
        if (count($parts) > 1) {
            $tmp = array_slice($parts, -4);
        } else {
            $tmp = $parts;
        }
        $fname = implode(' ', $tmp);
        $ext = strtolower(substr(strrchr($fname, '.'), 1));
        $fname = str_replace([':', '/', ' ', '.'], ['_', '_', '_', '_'],
                substr($fname, 0, strlen($fname) - strlen($ext) - 1)) . ".$ext";
        if ($res = copy(str_replace(' ', '%20', $url), $picsLibrary . '/' . $fname)) {
            return $fname;
        } else {
            flog('Copy error ' . $url . ' to ' . $picsLibrary . '/' . $fname);
            flog(error_get_last());
        }
        return false;
    }

    public static function getLocalFile($propID, $url)
    {
        $picsLibrary = IMG_LIBRARY . PROPERTIES;
        $picsPath = PROPERTIES_IMAGES;
        $parts = explode('/', $url);
        if (count($parts) > 1) {
            $tmp = array_slice($parts, -4);
        } else {
        ...
        $fname = implode(' ', $tmp);
        $ext = strtolower(substr(strrchr($fname, '.'), 1));
        $fname = str_replace([':', '/', ' ', '.'], ['_', '_', '_', '_'], $fname) . ".$ext";
        $res = false;
        if (file_exists($picsLibrary . $fname) && (filemtime($picsLibrary . $fname) + PICSCACHETIME < time())) {
            unlink($picsLibrary . $fname);
            $res = copy(str_replace(' ', '%20', $url), $picsLibrary . $fname);
        } else {
            if (!file_exists($picsLibrary . $fname)) {
                $res = copy(str_replace(' ', '%20', $url), $picsLibrary . $fname);
            } else {
                $res = true;
            }
        }
        return $res ? str_replace('//', '/', PROPS_DIR . $fname) : false;
    }

    public static function mobileDevice(
        $iphone = true,
        $ipad = false,
        $android = true,
        $opera = true,
        ...
        switch (true) { // using a switch against the following statements which could return true is more efficient than the previous method of using if statements
            ...
            case (preg_match('/ipod/i', $user_agent) || preg_match('/iphone/i',
                    $user_agent)); // we find the words iphone or ipod in the user agent
                $mobile_browser = $iphone; // mobile browser is either true or false depending on the setting of iphone when calling the function
                $status = 'Apple';
                $config->platform = "iPod";
            ...
            case (preg_match('/(pre\/|palm os|palm|hiptop|avantgo|plucker|xiino|blazer|elaine)/i',
                $user_agent)); // we find palm os in the user agent - the i at the end makes it case insensitive
                $mobile_browser = $palm; // mobile browser is either true or false depending on the setting of palm when calling the function
                $status = 'Palm';
                $config->platform = "Palm";
            ...
            case (preg_match('/(iris|3g_t|windows ce|opera mobi|windows ce; smartphone;|iemobile)/i',
                $user_agent)); // we find windows mobile in the user agent - the i at the end makes it case insensitive
            ...
            case (preg_match('/(mini 9.5|vx1000|lge |m800|e860|u940|ux840|compal|wireless| mobi|ahong|lg380|lgku|lgu900|lg210|lg47|lg920|lg840|lg370|sam-r|mg50|s55|g83|t66|vx400|mk99|d615|d763|el370|sl900|mp500|samu3|samu4|vx10|xda_|samu5|samu6|samu7|samu9|a615|b832|m881|s920|n210|s700|c-810|_h797|mob-x|sk16d|848b|mowser|s580|r800|471x|v120|rim8|c500foma:|160x|x160|480x|x640|t503|w839|i250|sprint|w398samr810|m5252|c7100|mt126|x225|s5330|s820|htil-g1|fly v71|s302|-x113|novarra|k610i|-three|8325rc|8352rc|sanyo|vx54|c888|nx250|n120|mtk |c5588|s710|t880|c5005|i;458x|p404i|s210|c5100|teleca|s940|c500|s590|foma|samsu|vx8|vx9|a1000|_mms|myx|a700|gu1100|bc831|e300|ems100|me701|me702m-three|sd588|s800|8325rc|ac831|mw200|brew |d88|htc\/|htc_touch|355x|m50|km100|d736|p-9521|telco|sl74|ktouch|m4u\/|me702|8325rc|kddi|phone|lg |sonyericsson|samsung|240x|x320|vx10|nokia|sony cmd|motorola|up.browser|up.link|mmp|symbian|smartphone|midp|wap|vodafone|o2|pocket|kindle|mobile|psp|treo)/i',
                $user_agent)); // check if any of the values listed create a match on the user agent - these are some of the most common terms used in agents to identify them as being mobile devices - the i at the end makes it case insensitive
            ...
            case ((strpos($accept, 'text/vnd.wap.wml') > 0) || (strpos($accept,
                        'application/vnd.wap.xhtml+xml') > 0)); // is the device showing signs of support for text/vnd.wap.wml or application/vnd.wap.xhtml+xml
                $mobile_browser = true; // set mobile browser to true
    ...
    // default cache header type is public, expire time is 1 month
    public static function cacheHeaders($type = 'public', $expireSeconds = 2592000, $md5sum = '')
    {
        header('Cache-Control: ' . $type . ', max-age=' . $expireSeconds . ', post-check=' . $expireSeconds . ', pre-check=' . $expireSeconds . '');
        header('Expires: ' . gmdate('D, d M Y H:i:s', strtotime('now +' . $expireSeconds . ' seconds')) . ' GMT');
        header('Last-Modified: ' . gmdate('D, d M Y H:i:s') . ' GMT');
        if ($md5sum) {
            header('ETag: ' . $md5sum);
            if (array_key_exists('HTTP_IF_NONE_MATCH', $_SERVER) && $_SERVER['HTTP_IF_NONE_MATCH'] == $md5sum) {
                header('HTTP/1.1 304 Not Modified');
            }
        }
    }

    public static function setCacheHeaders($meta = [])
    {
        $cacheMaxAge = intval($meta['cachetime']) * 60;
        switch ($meta['cachecontrol']) {
            case 'public':
                self::cacheHeaders('public', $cacheMaxAge);
                break;
            case 'private':
            case 'private_no_expire':
                self::cacheHeaders('private', $cacheMaxAge);
                break;
            case 'nocache':
                self::noCacheHeaders();
                break;
