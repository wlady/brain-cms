...
function RedirectToCachedFile() {
	...
	//bump:etag
	if ($nStat  = stat($phpThumb->cache_filename)) {
		$nModified  = $nStat['mtime'];
		$etag = md5($nStat['ino'].$nStat['mtime'].$nStat['size']); // apache like way 
	}
	//bumpend:etag
	...
	if ($phpThumb->config_nooffsitelink_enabled && @$_SERVER['HTTP_REFERER'] && !in_array(@$parsed_url['host'], $phpThumb->config_nooffsitelink_valid_domains)) {
		...
		//bump:etag
		header('ETag: "'.$etag.'"');
		header('Content-Length: '.$nStat['size']);
		header('Cache-Control: max-age=2592000, public');

		if (isset($_SERVER['HTTP_IF_NONE_MATCH']) && $etag == trim($_SERVER['HTTP_IF_NONE_MATCH'], '" ') && @$_SERVER['SERVER_PROTOCOL']) {
